...
public interface IIndex {
   ...
   * @param key
   *          the key to check
   * @return {@code true} if the series of key values contains only {@code null} values, or
   ...
   * @return the previous item stored in the index using the key, or {@code null} otherwise
   */
  @Nullable
  INodeItem put(@NonNull INodeItem item, @NonNull List<String> key);

  /**
   * Retrieve the item from the index that matches the key generated by evaluating the index's default
   * key field components against the provided item.
   *
   * @param item
   *          the item to store in the index
   * @param dynamicContext
  ...
  default INodeItem get(@NonNull INodeItem item, @NonNull DynamicContext dynamicContext) {
    return get(item, getKeyFields(), dynamicContext);
  }

  /**
   * Retrieve the item from the index that matches the key generated by evaluating the provided key
   * field components against the provided item.
   *
   * @param item
   ...
   * @param dynamicContext
   *          the Metapath evaluation context
   * @return the item with the matching key or {@code null} if no matching item was found
   */
  @Nullable
