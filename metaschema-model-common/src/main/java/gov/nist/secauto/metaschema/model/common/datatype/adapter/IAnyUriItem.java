...
public interface IAnyUriItem extends IAnyAtomicItem {
  @NotNull
  public static IAnyUriItem valueOf(@NotNull URI item) {
    IAnyUriItem retval;
    if (item.isAbsolute() || item.isOpaque()) {
      retval = MetaschemaDataTypeProvider.URI.newItem(item);
    } else {
      retval = MetaschemaDataTypeProvider.URI_REFERENCE.newItem(item);
    }
    return retval;
  }
