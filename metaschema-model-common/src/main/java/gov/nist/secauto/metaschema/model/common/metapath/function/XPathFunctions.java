...
public class XPathFunctions {
  ...
  public static INumericItem fnRound(INumericItem arg, IIntegerItem precisionItem) {
    ...
    if (precision >= 0) {
      ...
      if (arg instanceof IIntegerItem) {
        ...
        if (value.signum() == -1) {
          retval = IDecimalItem.valueOf(value.round(new MathContext(precision+value.precision()-value.scale(), RoundingMode.HALF_DOWN)));
        } else {
          retval = IDecimalItem.valueOf(value.round(new MathContext(precision+value.precision()-value.scale(), RoundingMode.HALF_UP)));
        }
      }
    } else if (precision < 0) {
  ...
  private static IIntegerItem roundNegativePrecision(INumericItem arg, int precision) {
    BigInteger value = arg.asInteger();
    ...
    return IIntegerItem.valueOf(result);
  }
}
