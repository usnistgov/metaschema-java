...
public abstract class AbstractFieldDefinition
  ...
  /**
   * Create a Metaschema field definition.
   * 
   * @param metaschema
  ...
  public JsonValueKeyTypeEnum getJsonValueKeyType() {
    JsonValueKeyTypeEnum retval = JsonValueKeyTypeEnum.NONE;
    if (hasJsonValueKey()) {
      FlagInstance<?> valueKeyFlag = getJsonValueKeyFlagInstance();
      if (valueKeyFlag != null) {
        retval = JsonValueKeyTypeEnum.FLAG;
      } else {
        retval = JsonValueKeyTypeEnum.STATIC_LABEL;
      }
    }
    return retval;
  }
  ...
  public Object getJsonValueKey() {
    Object retval = null;
    if (hasJsonValueKey()) {
      retval = getJsonValueKeyFlagInstance();
      if (retval == null) {
