...
public abstract class AbstractModelNodeItem<SEGMENT extends IModelPositionalPathSegment>
    extends AbstractBindingNodeItem<SEGMENT, IAssemblyNodeItem> implements IModelNodeItem {

  public AbstractModelNodeItem(Object value, SEGMENT segment, IAssemblyNodeItem parent) {
  ...
  public Stream<IFlagNodeItem> getChildFlags(Flag flag) {
    Stream<IFlagNodeItem> retval = Stream.empty();

    Object value = getValue();
    INamedDefinition definition = getPathSegment().getDefinition();
    if (definition instanceof ClassBinding) {
      ...
      if (flag.isName()) {
        ...
        if (instance != null) {
          Object instanceValue = instance.getValue(value);
          retval = instanceValue == null ? Stream.empty() : Stream.of(instance.newNodeItem(this, instanceValue));
        }
      } else {
        ...
        for (FlagProperty instance : classBinding.getFlagInstances().values()) {
          if (instanceMatcher.test(instance)) {
            ...
            if (instanceValue != null) {
              retval = Stream.concat(retval, Stream.of(instance.newNodeItem(this, instanceValue)));
            }
          }
        }
      }
