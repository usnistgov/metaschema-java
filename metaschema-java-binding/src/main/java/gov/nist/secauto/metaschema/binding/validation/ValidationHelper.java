  ...
  public @NotNull IValidationResult validate(@NotNull IBoundXdmNodeItem nodeItem, @NotNull IContentValidator validator) throws IOException {
    ...
    if (classBinding == null) {
      throw new IllegalArgumentException(
          String.format("The node '%s' is bound to a scalar value.", nodeItem.getMetapath()));
    }
    if (classBinding instanceof IAssemblyClassBinding) {
      IAssemblyClassBinding assemblyClassBinding = (IAssemblyClassBinding) nodeItem.getClassBinding();
      if (assemblyClassBinding.isRoot()) {
        ISerializer<?> deserializer
            = new DefaultJsonSerializer<>(DefaultBindingContext.instance(), assemblyClassBinding);
        deserializer.enableFeature(Feature.SERIALIZE_ROOT);

        try (StringWriter writer = new StringWriter(1024)) {
          deserializer.serialize(nodeItem.toBoundObject(), writer);
          writer.getBuffer();
      ...
      throw new IllegalArgumentException(String.format("The node '%s' not a root assembly.", nodeItem.getMetapath()));
    }
