...
class AssemblyNodeItemImpl
    extends AbstractModelNodeItem<IAssemblyPathSegment>
    implements IAssemblyNodeItem {

  public AssemblyNodeItemImpl(Object value, IAssemblyPathSegment segment, IAssemblyNodeItem parent) {
    super(value, segment, parent);
  }

  @Override
  public boolean isRootNode() {
    return getParent() == null && getPathSegment() instanceof IRootAssemblyPathSegment;
  }

  @Override
  public Stream<? extends IModelNodeItem> getChildModelInstances(ModelInstance modelInstance) {

    Stream<? extends IModelNodeItem> retval = Stream.empty();
    ...
    if (definition instanceof AssemblyClassBinding) {
      ...
      if (modelInstance.isName()) {
        ...
        if (instance != null) {
          retval = instance.getNodeItemsFromParentInstance(this);
        }
      } else {
        ...
        for (NamedModelProperty instance : classBinding.getModelInstances()) {
          if (instanceMatcher.test(instance)) {
            retval = Stream.concat(retval, instance.getNodeItemsFromParentInstance(this));
          }
        }
      }
    }
  ...
  public Stream<? extends INodeItem> getChildInstances(ExpressionEvaluationVisitor<INodeContext> visitor,
    ...
    // // get matching flag instances on this node
    // Collection<? extends IFlagInstance> flags = definition.getFlagInstances().values();
    // Stream<IFlagNodeItem> nodeItems = flags.stream().map(flagInstance -> {
    ...
    if (recurse) {
      ...
      Stream<? extends INodeItem> modelStream = modelInstances.stream().flatMap(modelInstance -> {
