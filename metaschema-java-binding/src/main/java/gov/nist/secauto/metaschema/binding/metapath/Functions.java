...
import gov.nist.secauto.metaschema.datatypes.metaschema.IUntypedAtomicItem;
import gov.nist.secauto.metaschema.datatypes.metaschema.InvalidTypeException;
import gov.nist.secauto.metaschema.datatypes.metaschema.MetapathDynamicException;
...
public class Functions {
  ...
  private static boolean fnBooleanInternal(IMetapathResult result) {
    ...
    if (result == null) {
      ...
      retval = gov.nist.secauto.metaschema.binding.metapath.Functions.fnBoolean((IItem)result);
    }
    return retval;
  }
  ...
  private static boolean fnBoolean(ISequence sequence) {
    ...
    if (!sequence.isEmpty()) {
      ...
      if (first instanceof INodeItem) {
        ...
        retval = gov.nist.secauto.metaschema.binding.metapath.Functions.fnBoolean(first);
      }
    }
    return retval;
  }
  ...
  public static boolean fnBoolean(IItem item) {
    ...
    if (item instanceof IBooleanItem) {
      ...
      retval = !item.toString().isBlank();
    } else if (item instanceof IUntypedAtomicItem) {
      IStringItem string = item.toStringItem();
      retval = !string.toString().isBlank();
  ...
  public static INumericItem toNumeric(IItem item) throws InvalidTypeException {
    ...
    if (item == null) {
      ...
      // atomize
      IAtomicItem atomicItem = Functions.fnDataItem(item);
  ...
  public static IAtomicItem fnDataItem(IItem item) {
    IAtomicItem retval;
    if (item instanceof IUntypedAtomicItem) {
      retval = item.toStringItem();
    } else if (item instanceof INodeItem) {
      retval = fnDataItem(item.toAtomicItem());
    } else {
      retval = (IAtomicItem) item;
    }
    return retval;
  }
