  ...
  private class Visitor extends NodeVisitorBase {
    ...
    protected boolean handleInlineElements(Node node) throws XMLStreamException {
      ...
      if (node instanceof Text) {
        ...
        handleSoftLineBreak((SoftLineBreak) node);
        retval = true;
      }
      return retval;
    }
    ...
    private void handleTypographicSmarts(TypographicQuotes node) throws XMLStreamException {
      ...
      if (node.getTypographicClosing() != null && !node.getTypographicClosing().isEmpty()) {
        getWriter()
            .add(eventFactory.createCharacters(mapEntity(node.getTypographicClosing().replaceFirst("&", "\\&"))));
      }
    }
    ...
    private void handleSoftLineBreak(@SuppressWarnings("unused") SoftLineBreak node) throws XMLStreamException {
      getWriter().add(eventFactory.createCharacters(" "));
    }
