...
public class BindingStrategyFactory implements IBindingStrategyFactory {
  @NonNull
  private static final BindingStrategyFactory SINGLETON = new BindingStrategyFactory();

  @NonNull
  public static BindingStrategyFactory instance() {
    return SINGLETON;
  }

  @Override
  public IClassBindingStrategy<?> newClassBindingStrategy(Class<?> clazz) {
    // IClassBindingStrategy<?> retval = null;
    // if (clazz.isAnnotationPresent(MetaschemaAssembly.class)) {
    // retval = AssemblyClassBindingImpl.createInstance(clazz, getBindingContext());
